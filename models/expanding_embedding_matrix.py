...
# see tokens generated for a single warao word
tokens = tok.convert_ids_to_tokens(tok('janoko')['input_ids'])  # jisabaya means 'house'
print(tokens)

warao_sent = 'Ka janoko sanukira'
tokens2 = tok.convert_ids_to_tokens(tok(warao_sent)['input_ids'])
print(tokens2)



# check initial translation: new tokens aren't useful yet since new words' embeddings haven't been trained yet
# generated_answer2 = tok.decode(model.generate(**tok(sent2, return_tensors='pt'), do_sample=True)[0]). # generate next best tokens
...
# add tokens
tok.add_tokens(warao_vocab_lst)
